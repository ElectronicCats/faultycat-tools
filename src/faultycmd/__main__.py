...
Uses an absolute import (`faultycmd.core.cli`) instead of a relative
one (`.core.cli`) so the PyInstaller bootloader — which loses package
context when executing a frozen `__main__.py` directly — can resolve
it. The absolute form works equally well under `python -m faultycmd`."""

from faultycmd.core.cli import _wrap_main
