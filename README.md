...
> `venv/pyvenv.cfg`. If you move or rename the repo checkout
> (or the `venv/` folder itself), the environment stops
...
results if the wire protocol drifted. See the firmware repo's
release docs for the full picture (version scheme, where the
version lives in the source tree, how the firmware advertises it,
